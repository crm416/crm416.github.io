...
The stated goal of Flow is to "find errors in JavaScript code with little programmer effort". Paraphrasing [Avik Chaudhuri](http://www.cs.umd.edu/~avik/), the project lead, Flow aims to enforce the benefits of a type system while maintaing the "feel" of JavaScript.
...
I wanted to give Flow a whirl, so I decided to integrate it into a small project to which I've contributed: [RCSS](https://github.com/chenglou/RCSS).
...
Flow allows you to provide a folder of interface files to be used during type checking, identified with `flow start --lib path/to/interfaces/`.

_(Note: Any file in path/to/interfaces will be included as an interface, [as long as it ends in .js](https://github.com/facebook/flow/blob/63f049dfafe4a53fa1d4875150c88bc7d7efab3f/src/common/files_js.ml#L34)--the specific naming scheme is irrelevant.)_

Interfaces should be filled with [declarations](flowtype.org/docs/declarations.html#_) and are primarily intended to be used for annotating third-party code. Any declarations you include in an interface will be global in the sense that any file Flow type checks can access them.

As an example interface, say I downloaded the foo-bar module from [npm](npmjs.org) which exports two properties: `reverse`, which reverses a string and `square`, which squares an integer. Then, I could add the following declaration to my interface file:
...
Notice that I'm able to [identify the module by its require-path](http://flowtype.org/docs/declarations.html#paths)--super elegant.
...
2. **Interfaces do not auto-reload** ([#87](https://github.com/facebook/flow/issues/87)). In effect, if you change or add an interface, the only way to reflect that change in the type system is by running `flow stop` and `flow start`. As I mentioned before, this can be a lengthy process, which makes interfaces inconvenient.
3. **Module interfaces don't support non-object exports** ([#73](https://github.com/facebook/flow/issues/73)). In my initial example, notice that I annotated the object properties exported by the module. If my module, for example, exported _just_ a function, there wouldn't be a good way to annotate it.
    ...
    But the downside here is that I then need to include this annotation wherever I import the reverse module.
...
An alternative would be to provide a declaration for `Function` in the same file that it's used, e.g.:
...
Per the [docs](http://flowtype.org/docs/running.html#_), you need to use Facebook's JSX transformer, packaged under the title [react-tools](https://www.npmjs.com/package/react-tools), to strip away Flow's type annotations.
...
The docs provide this more useful and more realistic example:

```
jsx --strip-types --harmony --watch src/ build/
```

I wanted to use this transform with [browserify](https://www.npmjs.com/package/browserify); luckily, the `--strip-types` option is built into [reactify](https://www.npmjs.com/package/reactify), the JSX browserify transform. Unluckily, it didn't work for unknown reasons.

In particular, I should've been able to run:

```
browserify -t reactify --strip-types index.js > out.js
```

But this produced some strange file that I couldn't quite parse. The [flow-typestrip](https://www.npmjs.org/package/flow-typestrip) package, on the other hand, worked as a drop-in replacement:

```
browserify -t reactify -t [flow-typestrip] index.js > out.js
```

This [transpilation](http://en.wikipedia.org/wiki/Source-to-source_compiler) step, while slightly inconvenient, is now the status quo for a lot of JS tooling. That said, there's been some interesting [discussion on GitHub](https://github.com/facebook/flow/issues/3) about finding ways to integrate Flow without the need for transpilation through the use of formatted comments.
...
The suggestions are to do this transformation in-place and use the output annotations for type checking. There are good ideas in that thread and I'm excited to see what the Flow team produces.

## Objects

In Flow, there are a few ways to annotate an object. The most basic, as you might guess, is to use a simple annotation:
...
Types can also be inferred from existing objects using the [`typeof` operator](http://flowtype.org/docs/typeof.html#_). This is a really, really cool feature. For example, Flow would catch an error here, as `x` and `y` have incorrect types for the second object:
...
There are a few 'gotchas' here that aren't spelled out in the documentation.
...
2. If you're using anything other than strings as your keys, your syntactic choices become very limited. For one thing, in general, non-string-literal keys are not supported by Flow. So if you want to use numbers as your keys, this won't work:
    ...
    Instead, you need to initialize an empty object and use bracket-syntax for assignment, like so:
...
3. This may be a bug, but Flow lets you include string-literal keys in the object constructor even if you include one of these map annotations. However, reading from or writing to these keys in the future always throws an error:
...
_(Note that I'm using a [union type](http://flowtype.org/docs/union-intersection-types.html#_) here.)_
...
Clearly, each of these options are awkward. I went with option #2, but was sort of unhappy that I had this single interface file with all these global type aliases. In a larger project, I could see this becoming inconvenient or difficult to maintain.
...
Things improve when you use [ES6 Classes](http://flowtype.org/docs/classes.html#_), as the classes can be exported and their types reused easily.
...
Much of this could be solved with better documentation and, with any luck, I'll be sending over some pull requests in the near future ([one down!](https://github.com/facebook/flow/pull/182)).
