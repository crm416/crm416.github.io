# What's Your Favorite Flavor (of JavaScript)?
...
TypeScript also amplifies some of the functional flavor of JavaScript (albeit, less-so than CoffeeScript), mainly by adding `(arg) => { // use arg }` lambdas. For example:
